...
async function initializeRuntime() {
  try {
    if (typeof loadPyodide !== "function") {
      throw new Error("Pyodide loader not found. Check the CDN script include.");
    }
    pyodide = await loadPyodide();
    runtimeStatus.textContent = "Loading core modules...";
    const requiredPackages = ["numpy", "pandas", "matplotlib", "micropip"];
    for (const pkg of requiredPackages) {
      await pyodide.loadPackage([pkg]);
    }
    runtimeStatus.textContent = "Loading optional modules...";
    try {
      await pyodide.loadPackage(["scikit-learn"]);
    } catch (_) {
      // Optional package: do not fail runtime when unavailable.
    }
    try {
...
function initializeLiveWallpaper() {
  const canvas = document.getElementById("liveWallpaperCanvas");
  if (!canvas) return;
  ...
  const blobs = [
    { x: 0.2, y: 0.25, r: 180, dx: 0.00008, dy: 0.00005, c: "120,210,255" },
    { x: 0.75, y: 0.2, r: 220, dx: -0.00006, dy: 0.00004, c: "255,155,72" },
    { x: 0.68, y: 0.72, r: 200, dx: -0.00007, dy: -0.00005, c: "180,120,255" },
    { x: 0.28, y: 0.78, r: 210, dx: 0.00005, dy: -0.00006, c: "109,255,198" }
  ];
  ...
  function draw() {
    ...
    blobs.forEach((b) => {
      ...
      const gradient = ctx.createRadialGradient(gx, gy, 0, gx, gy, b.r);
      gradient.addColorStop(0, `rgba(${b.c},0.27)`);
      gradient.addColorStop(1, `rgba(${b.c},0)`);
